...
function removeFromCart(productId) {
    ...
    if (item) {
        ...
        localStorage.setItem('cart', JSON.stringify(cart));
        updateCartDisplay();
        showNotification(`${item.name} verwijderd uit winkelwagen`);
    }
}
...
function updateQuantity(productId, newQuantity) {
    ...
    if (item) {
        ...
        if (newQuantity > oldQuantity) {
            showNotification(`${item.name} hoeveelheid verhoogd naar ${newQuantity}`);
        } else if (newQuantity < oldQuantity) {
            showNotification(`${item.name} hoeveelheid verlaagd naar ${newQuantity}`);
        }
    }
}
...
function updateCartDisplay() {
    ...
    const cartHTML = validCartItems.map(item => `
        <div class="flex items-center space-x-2 py-2 border-b border-gray-200 last:border-b-0">
            <img src="${item.image}" alt="${item.name}" class="w-8 h-8 object-contain rounded flex-shrink-0">
            ...
            <div class="flex items-center space-x-1">
                <button onclick="updateQuantity(${item.id}, ${item.quantity - 1})" class="text-gray-400 hover:text-gray-600 p-1">
...
function showNotification(message) {
    // Maak notificatie element - een nieuw HTML element
    ...
    // Animate in - maak het element zichtbaar met een mooie animatie
    setTimeout(() => {
        notification.classList.remove('translate-x-full');
    }, 100);
    
    // Animate out en verwijder - na 3 seconden verdwijnt het bericht weer
    setTimeout(() => {
...
function resetCart() {
    ...
    updateCartDisplay();
    showNotification('Winkelwagen is gereset');
}

// Functie om winkelwagen te legen
function clearCart() {
    if (cart.length === 0) {
        showNotification('Winkelwagen is al leeg');
        return;
    }
    ...
    localStorage.removeItem('cart');
    updateCartDisplay();
    showNotification(`${itemCount} product${itemCount > 1 ? 'en' : ''} verwijderd uit winkelwagen`);
}
