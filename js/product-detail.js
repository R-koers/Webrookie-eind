...
function generateProductDescription(product) {
    ...
    let description = `Deze ${categoryName.toLowerCase()} van ${product.name} biedt uitstekende prestaties voor uw PC build. `;
    
    if (product.specifications) {
        const specs = Object.values(product.specifications);
        if (specs.length > 0) {
            description += `Met specificaties zoals ${specs.slice(0, 3).join(', ')} levert dit product betrouwbare en snelle prestaties. `;
        }
    }
    
    description += `Perfect voor gaming, werkstations of algemeen gebruik. Leverbaar met gratis verzending en 2 jaar garantie.`;
...
function displayRelatedProducts(currentProduct) {
    ...
    const relatedProducts = allProducts
        .filter(p => p.id !== currentProduct.id && getProductCategory(p) === currentCategory)
        .slice(0, 4);
    
    if (relatedProducts.length === 0) {
        relatedContainer.innerHTML = '<p class="text-gray-500 text-center col-span-full py-8">Geen gerelateerde producten gevonden</p>';
        return;
    }
    ...
    const relatedHTML = relatedProducts.map(product => `
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow duration-300">
            ...
            <div class="p-4">
                ...
                <p class="text-lg font-bold text-blue-950 mb-3">€${product.price.toFixed(2).replace('.', ',')}</p>
                <a href="product.html?id=${product.id}" class="block w-full bg-blue-950 hover:bg-blue-900 text-white text-center py-2 px-4 rounded-lg transition-colors duration-300 text-sm font-medium">
                    Bekijk product
                </a>
            </div>
        </div>
