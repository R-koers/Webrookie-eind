...
let allProducts = [];        // Hier slaan we alle producten op die we van de server krijgen
let filteredProducts = [];   // Hier slaan we de producten op die gefilterd zijn (bijvoorbeeld alleen CPU's)
...
async function loadProducts() {
    try {
        ...
        localStorage.setItem('hasAdminEdits', 'false'); // Markeer als niet admin-bewerkt
...
function createProductCard(product) {
    ...
    return `
        <div class="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-lg transition-all duration-300 border border-gray-200 group product-card">
            <div class="w-full h-[300px] flex justify-center items-center p-4 bg-gray-50 overflow-hidden">
                <img src="${product.image}" alt="${product.name}" class="max-w-full max-h-full w-auto h-auto object-contain group-hover:scale-105 transition-transform duration-300" style="max-width: 100%; max-height: 100%;" />
            </div>
            <div class="p-4 space-y-3">
                <h3 class="text-lg font-bold text-gray-800 line-clamp-2 min-h-[56px] group-hover:text-blue-950 transition-colors">
                    ${product.name}
                </h3>
                ...
                <div class="flex flex-row gap-2 justify-between">
                    ...
                    <button data-product-id="${product.id}" class="add-to-cart-btn bg-blue-950 hover:bg-blue-900 text-white py-2 px-4 rounded-lg transition-colors duration-300 text-sm font-medium flex items-center justify-center gap-2">
...
function getProductCategory(product) {
    ...
    return 'other'; // Als het niks van bovenstaande is, is het "anders"
}
...
function filterAndSortProducts(category = '', sortBy = 'name-asc', searchTerm = '') {
    ...
    return filtered; // Geef de gefilterde en gesorteerde producten terug
}

// Functie om best sellers weer te geven (top 4 producten)
function displayBestSellers() {
    const bestSellers = allProducts.slice(0, 4); // Eerste 4 producten als best sellers
